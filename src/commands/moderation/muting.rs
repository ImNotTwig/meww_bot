...
pub struct UnmutedTime {
    pub unmuted_time: BTreeMap<String, String>,
}
...
pub async fn mute(
    ctx: Context<'_>,
    #[description = "User to mute."] mut member: sr::Member,
    ...
    if mute_roles.contains_key(&guild_id) == false {
        ...
        ctx.say(format!(
            "{} does not have a mute role, please set one uaing {}muterole",
    ...
    if member.roles.contains(&RoleId::from(
        mute_roles.get(&guild_id).unwrap().parse::<u64>().unwrap(),
    )) {
    ...
    if &time == &None {
        ...
        let mut unmute_times_dict =
            serde_json::from_str::<BTreeMap<String, UnmutedTime>>(&unmute_json)?;

        // if the unmute_times_dict doesnt have the guild inserted yet
        if unmute_times_dict.contains_key(&guild_id) == false {
            unmute_times_dict.insert(
                guild_id.clone(),
                UnmutedTime {
                    unmuted_time: BTreeMap::new(),
                },
            );
        }
        ...
        users_muted
            .unmuted_time
            .insert(user_id, time_muted_as_string);
        ...
        let mut unmute_times =
            fs::File::create("./src/commands/moderation/unmuted_times.json").unwrap();
        write!(
            unmute_times,
            "{}",
    ...
    if reason == None {
        if time_units != 0 {
            ...
            ctx.say(format!(
                "{} has been muted for {} {}.",
                &member.user.name, &time_units, &timeframe
            ))
            .await?;
        } else {
            ctx.say(format!("{} has been muted.", &member.user.name))
                .await?;
        }
    } else if reason != None {
        if time_units != 0 {
...
pub async fn unmute(
    ctx: Context<'_>,
    #[description = "User to unmute."] mut member: sr::Member,
) -> Result<(), Error> {
    ...
    if mute_roles.clone().contains_key(&guild_id) == false {
        let config = read_config().unwrap();
        ctx.say(format!(
            "{} does not have a mute role, please set one using {}muterole",
            &ctx.guild().unwrap().name,
            config.prefix
        ))
        .await?;
    ...
    ctx.say(format!("{} has been un-muted", member.user.name))
        .await?;
...
pub async fn muterole(
    ctx: Context<'_>,
    #[description = "Role to set."] role: sr::Role,
) -> Result<(), Error> {
    ...
    let mut mute_roles_file =
        fs::File::create("./src/commands/moderation/mute_roles.json").unwrap();
    write!(
        mute_roles_file,
        "{}",
        String::from_utf8(ser.into_inner()).unwrap()
    ...
    ctx.say(format!(
        "{} has been set as the mute role for {}",
        role.name,
        ctx.guild().unwrap().name
    ))
    .await?;
